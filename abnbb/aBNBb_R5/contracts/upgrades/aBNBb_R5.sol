...
contract aBNBb_R5 is OwnableUpgradeable, ERC20ModUpgradeable, IBondToken_R2 {
    /**
     * Variables
     */
        ...
        returns (uint256)
    {
        ...
        uint256 sum = 0;
        uint8 currentIndex = indexOfRatio(block.timestamp);
        if (
            _ratio != historicalRatios[currentIndex] &&
            _ratio == historicalRatios[currentIndex - 1]
        ) {
            --currentIndex;
        }
        uint8 index = currentIndex;
        for (uint8 i; i < day; ++i) {
            ...
            --index;
            sum +=
                ((historicalRatios[index] - historicalRatios[oldIndex]) *
                    10**20) /
                historicalRatios[index];
        }
        return (sum / uint256(day)) * 365;
    }
    ...
    function updateRatio(uint256 totalRewards) external onlyOperator {
        ...
        if (block.timestamp - _lastUpdate > 1 days - 1 minutes) {
            uint256 index = indexOfRatio(block.timestamp);
            historicalRatios[index] = _ratio;
